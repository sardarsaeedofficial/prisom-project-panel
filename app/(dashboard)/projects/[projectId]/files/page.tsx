...
import { StorageFileManager } from "@/components/projects/file-manager";
...
// ── UI helpers (for GitHub/read-only tree) ────────────────────────────────────
...
export default async function ProjectFilesPage({ params }: Props) {
  ...
  // Stats from whichever source is active (only needed for the read-only panel)
  ...
  const totalFiles = dbFileItems.length;
  const totalDirs = dbDirItems.length;
  const totalCode = dbCodeFiles.length;

  // Tree entries for the read-only GitHub tree
  const treeEntries = dbFiles;
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        {useStorageFiles ? (
          /* ── Interactive file manager for uploaded/blank projects ── */
          <StorageFileManager
            projectId={projectId}
          ...
          /* ── Read-only tree for GitHub-linked or empty projects ── */
          <>
            <aside className="w-60 border-r bg-muted/20 flex flex-col overflow-hidden shrink-0">
              ...
              <div className="flex-1 overflow-y-auto">
                {treeEntries.length === 0 ? (
                  ...
                  <div className="py-1">
                    {treeEntries.slice(0, 500).map((file) => {
                    ...
                    {treeEntries.length > 500 && (
                      <p className="text-xs text-muted-foreground px-3 py-2">
                        … and {(treeEntries.length - 500).toLocaleString()} more
                      </p>
                    )}
                  </div>
                )}
