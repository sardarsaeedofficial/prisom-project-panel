...
export function generateHandoffMarkdown(
  ...
  return [
    ...
    buildHelpCenterSection(),          // Sprint 81
    buildSmartImportSection(),         // Sprint 85
...
// ── Sprint 81: Help Center section ────────────────────────────────────────────

function buildHelpCenterSection(): string {
  return [
    "---",
    "",
    "## Project Help Center and Knowledge Base",
    "",
    "**Sprint 81/82 — Project Help Center + Knowledge Base README Assistant**",
    "",
    "### Purpose",
    "",
    "The Project Help Center generates a living README and searchable knowledge base from the panel codebase.",
    "Operators can search sections, ask natural-language questions, and download three complete documentation exports.",
    "The system never invents information — all answers come from the generated knowledge base sections.",
    "",
    "### Help Center Route",
    "",
    "Navigate to: **`/projects/[projectId]/help`**",
    "",
    "Compact quick-access cards also appear on: **Releases, Runbook, Settings, Migration, Monitoring, Publishing**.",
    "",
    "### Generated Documentation Files",
    "",
    "| Export File | Contents |",
    "| --- | --- |",
    "| `PROJECT_KNOWLEDGE_BASE.md` | Quick reference (routes, actions, export files, frameworks) + 15 knowledge sections + safety rules + excluded paths |",
    "| `PROJECT_FILE_INVENTORY.md` | Per-file listing by category (page / server_action / component / library / export / schema) with language, size, and summary |",
    "| `PROJECT_METHODS_AND_RESOURCES.md` | Deployment commands, smoke checks, safety rules, server actions table, exported functions table, routes table, package imports |",
    "| `PROJECT_DEEP_MAP.md` | Route map, action map (with permissions + audit events), export dependency map, node summary |",
    "| `OPERATOR_SOP_LIBRARY.md` | 14 operator SOPs — deploy, smoke checks, go/no-go, launch checklist, monitoring, logs, exports, Sardar health, rollback, handover, doc updates |",
    "| `TROUBLESHOOTING_PLAYBOOK.md` | 14 troubleshooting playbooks — panel down, Sardar issues, build failures, export errors, PM2 crash, domain/SSL, payments |",
    "",
    "### Knowledge Base Scope (what is scanned)",
    "",
    "| Directory | Contents |",
    "| --- | --- |",
    "| `app/` | Pages, layouts, API routes, server actions |",
    "| `components/` | UI components and panels |",
    "| `lib/` | Library modules, services, types, exports |",
    "| `prisma/` | Database schema |",
    "| `scripts/` | Build and utility scripts |",
    "| `public/` | Static assets |",
    "| Root files | README.md, package.json, tsconfig.json, next.config.ts, tailwind.config.ts, pnpm-workspace.yaml |",
    "",
    "### Excluded Paths (never scanned — hard exclusions)",
    "",
    "| Path | Reason |",
    "| --- | --- |",
    "| `.env`, `.env.*` | Production secrets |",
    "| `node_modules/` | Installed packages — no value for documentation |",
    "| `.git/` | Version control internals |",
    "| `.next/` | Build output |",
    "| `dist/`, `build/`, `coverage/` | Output directories |",
    "| `storage/backups/` | Production backup data |",
    "| `*.pem`, `*.key`, `*.crt`, `*.cer`, `*.p12` | SSL/TLS certificates |",
    "| `*.log` | May contain session tokens or sensitive runtime data |",
    "| Hidden files (`.anything`) | System and config secrets |",
    "",
    "### Search and Ask Help Behaviour",
    "",
    "- **Search**: Enter keywords to find ranked knowledge sections — scoring boosts exact phrase match in title (+30), all words in title (+20), exact phrase in keywords (+18), important domain terms (deploy, pm2, sardar, export, etc.)",
    "- **Ask**: Ask a natural-language question — the system routes to the most relevant knowledge sections and answers extractively",
    "- **Common question routing**: Deploy questions → deployment section; Sardar questions → sardar section; safety questions → safety section; server action questions → server_actions section",
    "- **Missing info**: If the answer is not in the knowledge base, the system explicitly says so and suggests alternatives (PM2 logs, SSH, etc.)",
    "- **No hallucination**: The system never invents answers — all output comes from the generated knowledge base",
    "",
    "### No-Secret Guarantee",
    "",
    "- All file content is passed through `redactHelpContent()` before any processing",
    "- Redaction patterns cover: `DATABASE_URL`, `NEXTAUTH_SECRET`, `SESSION_SECRET`, `STRIPE_SECRET_KEY`, `RESEND_API_KEY`, `JWT_SECRET`, `postgres://` connection strings, `mongodb://`, `redis://`, `-----BEGIN` PEM blocks, generic `password=`, `api_key=`, `secret=` assignments",
    "- A `containsSecretPattern()` gate also filters any snippet before surfacing it in search results or ask answers",
    "- Only variable names (not values), file paths, function names, and route paths appear in any output",
    "",
    "### Safety Rules (enforced by the Help Center)",
    "",
    "- No production mutation, DNS changes, PM2 restarts, or DB migrations are triggered",
    "- The Help Center is read-only documentation generation only",
    "- Safety notes appear automatically in ask answers whenever PM2, DNS, nginx, Sardar, Doorsteps, DB migrations, or Stripe are mentioned",
    "",
    "### Recommended Operator Questions for Handoff",
    "",
    "| # | Question | Expected Section |",
    "| --- | --- | --- |",
    "| 1 | What is the deployment procedure for this project? | Deployment Commands |",
    "| 2 | What PM2 processes must never be restarted from UI? | Safety Rules |",
    "| 3 | How do I run smoke checks after a deploy? | Deployment Commands |",
    "| 4 | Where are the go-live exports stored? | Exports |",
    "| 5 | How does the Sardar migration workflow work? | Sardar Migration |",
    "| 6 | What confirmation phrases are used in destructive actions? | Safety Rules |",
    "| 7 | Which env var names are required for production? | Environment Variables |",
    "| 8 | What server actions exist and where are they? | Server Actions |",
    "| 9 | What should I not touch? | Safety Rules |",
    "| 10 | What languages and frameworks are used? | Languages and Frameworks |",
    "",
    "### Implementation Files",
    "",
    "| File | Purpose |",
    "| --- | --- |",
    "| `lib/help-center/help-center-types.ts` | TypeScript types for all Help Center objects |",
    "| `lib/help-center/help-redaction.ts` | Secret redaction + exclusion rules |",
    "| `lib/help-center/project-file-inventory.ts` | Node.js file scanner — classifies and summarizes all safe project files |",
    "| `lib/help-center/project-knowledge-builder.ts` | Builds 15 knowledge sections from the file inventory |",
    "| `lib/help-center/help-search-index.ts` | Keyword scoring search (deterministic, no AI required) |",
    "| `lib/help-center/help-answer-service.ts` | Natural-language Q&A with canned routing and extractive answers |",
    "| `lib/help-center/help-center-export.ts` | Generates the three export markdown files |",
    "| `app/actions/help-center.ts` | Six server actions (generate, search, ask, export ×3) |",
    "| `components/projects/help-center-panel.tsx` | Generate + tabs + export panel |",
    "| `components/projects/help-search-panel.tsx` | Search + ask + confidence badge panel |",
    "| `app/(dashboard)/projects/[projectId]/help/page.tsx` | Dedicated Help Center page |",
    "",
  ].join("\n");
}

// ── Sprint 85: Smart Import section ──────────────────────────────────────────

function buildSmartImportSection(): string {
  return [
    "---",
    "",
    "## Smart Import and Auto-Go-Live Readiness",
    "",
    "**Sprint 85 — One-Click Smart Import + Auto-Go-Live Wizard**",
    "",
    "### What Smart Import Detects",
    "",
    "Smart Import scans the project source directory and identifies:",
    "",
    "- **Package manager** — pnpm, npm, yarn, bun",
    "- **pnpm workspace** — detects `pnpm-workspace.yaml` + `artifacts/` structure",
    "- **Frontend framework** — Vite/React, Next.js, SvelteKit, Astro",
    "- **API backend** — Express, Fastify, Hono, NestJS",
    "- **Static output path** — `artifacts/sardar-security/dist/public` or `dist/`",
    "- **Database tool/provider** — Drizzle/PostgreSQL, Prisma/PostgreSQL",
    "- **Required env var names** — by name only, never values",
    "- **Replit-specific markers** — `.replit`, `pnpm-workspace.yaml`, `artifacts/api-server`",
    "- **Health endpoint** — `/api/healthz` for API services",
    "",
    "### Preset Priority",
    "",
    ...
    "   - Health: `/api/healthz`, Mode: `static_plus_api`",
    "   - Static: `artifacts/sardar-security/dist/public`",
    "2. Generic Replit React + Express pnpm workspace (medium confidence)",
    "3. Next.js app (high confidence)",
    "4. Vite static app (medium confidence)",
    "5. API-only Node service (medium confidence)",
    "6. Manual review required (low confidence)",
    "",
    "### Preview Checks",
    "",
    "Smart Import can run HTTP checks against the deployed project:",
    "",
    "- `GET /api/healthz` — API health endpoint",
    "- `GET /` — Frontend root (expects HTML or SPA index)",
    "- `GET /products` — SPA route fallback (expects 200 with index.html)",
    "",
    "If the API responds but `/` returns `Cannot GET /`, Smart Import reports:",
    "> API is healthy, but frontend static output is not served.",
    ...
    "| Code | Error | Fix | Auto |",
    "| --- | --- | --- | --- |",
    "| `USE_PNPM` | npm install failed | Switch to pnpm preset | Yes |",
    "| `STATIC_NOT_SERVED` | Cannot GET / | Apply static_plus_api routing | Yes |",
    "| `SOURCE_MAPS_FLAG` | --enable-source-maps blocked | Normalize start command | Yes |",
    "| `WRONG_HEALTH_PATH` | Health / fails | Change to /api/healthz | Yes |",
    "| `PNPM_IGNORED_BUILDS` | esbuild blocked | Add onlyBuiltDependencies | No |",
    "| `CHAINED_COMMAND` | && not allowed | Use pnpm run build | Yes |",
    "| `MISSING_DATABASE_URL` | No DATABASE_URL | Add to env vars | No |",
    "| `MISSING_STRIPE_SECRET` | No STRIPE_WEBHOOK_SECRET | Add to env vars | No |",
    "",
    "### Safety Rules",
    "",
    "- **Go-live requires manual confirmation** — Smart Import never auto-promotes to production.",
    "- No secrets are shown or stored in reports.",
    "- No DB migrations are triggered automatically.",
    "- No DNS or nginx mutations happen from the Import panel.",
    "- No PM2 restarts are triggered by Smart Import (only normal deploy flow).",
    "- Doorsteps/LocalShop project is not affected.",
    "",
    "### Smart Import Routes",
    "",
    "| Route | Purpose |",
    "| --- | --- |",
    "| `/projects/[id]/import` | Smart Import panel (top of page) + Source Intake |",
    "| `/projects/[id]/publishing` | Compact Smart Import card near deployment config |",
    "",
  ].join("\n");
}
