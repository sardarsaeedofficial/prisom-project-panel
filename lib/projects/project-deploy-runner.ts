...
async function pm2StartFresh(
  ...
  // PORT and NODE_ENV always override whatever the project has set
  ...
  // Write PM2 ecosystem file with all env vars
  // The file is in the release dir (server-private — not web-accessible)
  const ecoPath = path.join(releasePath, "prisom.ecosystem.cjs");
  ...
  try {
    await fs.writeFile(ecoPath, ecoContent, "utf8");
  } catch (e) {
    return {
      ok: false,
  ...
  log.push(`▶ pm2 start prisom.ecosystem.cjs (${pm2Name})`);
  const startResult = await runCommand("pm2", ["start", ecoPath], {
    cwd: releasePath,
    timeoutMs: 30_000,
  });
