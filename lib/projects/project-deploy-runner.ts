...
export type RouteMode = "fullstack_node" | "static_plus_api" | "static_only" | "api_only";
...
export interface DeployConfig {
  ...
  healthPath: string;
  nodeEnv: string;
  /** Decrypted project env vars — never log these. */
  envVars?: Record<string, string>;
  /** Route/serving mode — governs nginx + static publishing. */
  routeMode?: RouteMode;
  /** Relative path inside release to the built static frontend (e.g. "dist"). */
  staticOutputDir?: string | null;
  /** API prefix for static_plus_api mode (default "/api"). */
  apiPrefix?: string;
}
...
export interface DeployRunnerResult {
  ...
  /** Absolute path where static files were published (if routeMode has static). */
...
/**
 * Builds a PM2 ecosystem config object for use in prisom.ecosystem.cjs.
 */
function buildEcosystemApp(
  pm2Name: string,
  ...
  if (parsed.binary === "node") {
    return {
      name:        pm2Name,
      script:      path.resolve(releasePath, parsed.args[0]),
      cwd:         releasePath,
      interpreter: "node",
      env:         envVars,
    };
  ...
  return {
    name:        pm2Name,
    script:      parsed.binary,
    args:        parsed.args.join(" "),
    cwd:         releasePath,
    interpreter: "none", // Required for package managers (npm/pnpm/yarn)
    env:         envVars,
  };
}

/**
 * Deletes any existing PM2 process with `pm2Name`, writes a PM2 ecosystem
 * config file (so env vars are persisted in the daemon), then starts it.
 * Deleting first ensures the new `--cwd` and env vars take effect.
 *
 * The file MUST be named `ecosystem.config.cjs` — PM2 uses the filename to
 * decide whether to treat the file as an ecosystem config or a plain script.
 * Any other name causes PM2 to run `node <file>` directly, ignore the `apps`
 * block, name the process after the filename, and inject no env vars at all.
 */
async function pm2StartFresh(
  ...
  const injectedNames = Object.keys(fullEnv);
  log.push(`▶ Injecting env vars: ${injectedNames.join(", ")}`);

  // IMPORTANT: PM2 requires the file to be named `ecosystem.config.js` or
  // `ecosystem.config.cjs` to be loaded as an ecosystem config file.
  // Any other name causes PM2 to exec the file as a Node script.
  const ecoPath = path.join(releasePath, "ecosystem.config.cjs");
  const ecoApp  = buildEcosystemApp(pm2Name, parsedStart, releasePath, fullEnv);
  const ecoContent =
  ...
  try {
    ...
    // Ensure mode is 600 even if umask overrides writeFile's mode parameter
    await fs.chmod(ecoPath, 0o600);
  } catch (e) {
    return {
      ok: false,
      output: `✗ Failed to write PM2 ecosystem file: ${(e as Error).message}`,
    };
  }
  ...
  // Use --only <pm2Name> so PM2 only starts the correct app from the config,
  // and --update-env so env changes on restart are always picked up.
  log.push(`▶ pm2 start ecosystem.config.cjs --only ${pm2Name}`);
  const startResult = await runCommand(
    "pm2",
    ["start", ecoPath, "--only", pm2Name, "--update-env"],
    { cwd: releasePath, timeoutMs: 30_000 }
  );
...
export async function runProjectDeployment(
  ...
  try {
    ...
    // Base env passed to install + build steps.
    ...
    // ── 5. PM2 start (skipped for static_only mode) ───────────────────────
    const mode = config.routeMode ?? "fullstack_node";
    let publicStaticPath: string | undefined;
    ...
    if (mode !== "static_only") {
      ...
      const pm2Result = await pm2StartFresh(
        ...
        config.port,
        config.nodeEnv,
        config.envVars // decrypted project env vars
      );
      ...
      log("\n⚙ Route mode: static_only — skipping PM2 start");
    }

    // ── 7. Static publishing (static_plus_api / static_only) ──────────────
    if (
      (mode === "static_plus_api" || mode === "static_only") &&
      config.staticOutputDir
    ) {
      log(`\n▶ Publishing static files from "${config.staticOutputDir}" → /var/www/…`);
      const staticResult = await publishStaticSite(
        releasePath,
        config.slug,
        deploymentRef,
        config.staticOutputDir
      ...
      if (!staticResult.ok) {
        // Static publish failure is non-fatal — warn but don't fail the deployment
        log(`⚠ Static publish failed: ${staticResult.error}`);
        log("  App is running — publish the static files manually to enable nginx serving.");
      } else {
        publicStaticPath = staticResult.publishPath;
        log(`✓ Static files published to ${publicStaticPath}`);
      }
    }
