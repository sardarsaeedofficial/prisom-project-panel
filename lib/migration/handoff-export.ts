...
function buildHelpCenterSection(): string {
  return [
    ...
    "",
    "**Sprint 81 — Project Help Center + Knowledge Base README Assistant**",
    "",
    "### Purpose",
    "",
    "The Project Help Center generates a living README and searchable knowledge base from the panel codebase.",
    "Operators can search sections, ask questions, and export three documentation files.",
    "",
    "### Access",
    "",
    "Navigate to: `/projects/[projectId]/help`",
    "",
    "Compact Help Center cards are also visible on: Runbook, Settings, Releases, Migration, Monitoring, and Publishing pages.",
    "",
    "### Knowledge Base Scope",
    "",
    "The scanner reads safe directories only:",
    "",
    "| Directory | Contents |",
    "| --- | --- |",
    "| `app/` | Pages, layouts, API routes, server actions |",
    "| `components/` | UI components and panels |",
    "| `lib/` | Library modules, services, types |",
    ...
    "- `*.log` (storage) — may contain sensitive data",
    "",
    "### Available Exports",
    "",
    "| Export File | Contents |",
    "| --- | --- |",
    "| `PROJECT_KNOWLEDGE_BASE.md` | All 15 knowledge sections — overview, architecture, routes, actions, components, exports, deployment, safety, etc. |",
    "| `PROJECT_FILE_INVENTORY.md` | Full per-file listing grouped by category with language and size |",
    "| `PROJECT_METHODS_AND_RESOURCES.md` | All exported functions, server actions, routes, and package imports |",
    "",
    "### Search and Ask Help",
    "",
    "- **Search**: Enter keywords to find matching knowledge sections (e.g. `deploy`, `server actions`, `Sardar`, `exports`)",
    "- **Ask**: Ask a natural language question — the system answers from generated sections only, never invents information",
    "- **Missing info**: If the answer is not in the knowledge base, the system explicitly says so",
    "",
    "### Example Questions",
    "",
    ...
    "",
    "### Safety and No-Secret Rules",
    "",
    "- No secret values are exposed in any export or answer",
    "- Only variable names (not values) are shown",
    "- Redaction patterns cover DATABASE_URL values, STRIPE keys, JWT secrets, passwords, tokens",
    "- All scan output is read-only documentation",
    "- No production mutation, DNS changes, PM2 restarts, or DB migrations are triggered by the Help Center",
    "",
    "### Suggested Operator Questions to Ask on Handoff",
    "",
    "1. What is the deployment procedure for this project?",
    "2. What PM2 processes must not be touched?",
    "3. Where are the go-live exports stored?",
    "4. How does the Sardar migration workflow work?",
    "5. What confirmation phrases are used in destructive actions?",
    "6. How do I check if the panel is healthy after a deploy?",
    "7. Which env vars are required for production?",
    "8. Where are audit logs stored?",
    "",
  ].join("\n");
}
